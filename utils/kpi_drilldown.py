...
def render_kpi_suite(filtered, get_income):
    ...
    # =========================================================
    # 🔥 14️⃣ Avg Monthly Category Spend (Total + % Share)
    # =========================================================
    with st.expander("📊 Average Monthly Category Spend (Total + %)"):
        ...
        # Avg monthly spend per category
        avg_cat = (
            cat_month
        ...
        # Merge
        cat_final = avg_cat.merge(total_cat, on="category")

        # Percentage share
        grand_total = cat_final["total_spend"].sum()
        cat_final["percent"] = (cat_final["total_spend"] / grand_total * 100).round(1)

        # Label text
        cat_final["label"] = (
            "₹" + cat_final["avg_monthly"].round(0).astype(int).astype(str)
            + " | "
            + cat_final["percent"].astype(str) + "%"
        )

        # Bar chart
        bars = alt.Chart(cat_final).mark_bar(
            color="#42A5F5",
            size=36
        ).encode(
            x=alt.X("category:N", sort="-y", title="Category"),
            y=alt.Y("avg_monthly:Q", title="Avg Monthly Spend (₹)"),
            tooltip=[
                "category:N",
                alt.Tooltip("avg_monthly:Q", title="Avg Monthly", format=",.0f"),
                alt.Tooltip("total_spend:Q", title="Total Spend", format=",.0f"),
                alt.Tooltip("percent:Q", title="Spend %")
            ]
        )

        # Value labels
        labels = alt.Chart(cat_final).mark_text(
            dy=-10,
            fontSize=11,
            fontWeight="bold",
            color="white"
        ...
        st.write("---")
        st.success(
            f"💸 Highest Avg Monthly Category → "
            f"{cat_final.loc[cat_final.avg_monthly.idxmax(), 'category']} "
            f"(₹{cat_final.avg_monthly.max():,.0f})"
        )
